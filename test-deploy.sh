...
sshpass -p $INSTALL_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $INSTALL_SSH_USERNAME@$INSTALL_HOST "rm /usr/local/GrowSense/Installer/GrowSenseIndex.zip && rm /usr/local/GrowSense/Index/ -R"
...
if [ ! $DEPLOYED_VERSON -eq $CURRENT_VERSION ]; then
